...
- 首轮 SG 验证表明 Render/Cloudflare 可能代答 WebSocket 控制帧 Pong，因此仅依赖标准 Ping/Pong 不能证明端到端浏览器仍在线。

## 功能需求
- **FR-1**：服务端必须周期性发送标准 WebSocket Ping 和带序号的应用层心跳，并在客户端未及时返回对应应用层 ACK 时强制终止连接和释放会话。
- **FR-2**：客户端正确返回对应应用层 ACK 时，心跳不得误杀会话；代理代答控制帧 Pong 不得续租终端会话。
...
## 假设
- 浏览器会按照 WebSocket 标准自动响应服务端 Ping，并由网络桥脚本显式响应应用层心跳。
...
### AC-1：失活会话自动回收
- **Type**：`rule`
- **Given**：一个已接受的 WebSocket 客户端不响应应用层心跳，即使代理仍返回控制帧 Pong。
- **When**：经过两个心跳检查周期。
